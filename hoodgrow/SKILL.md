---
name: hoodgrow
description: "Pay-per-call Robinhood Chain stock-token data — live price, corporate-action adjusted supply, and pending/historical corporate actions (splits, dividends) for every listed token — settled per query in USDC on Base via x402, no signup or API key."
tags: [stock-tokens, tokenized-equities, robinhood-chain, defi, data, corporate-actions, rwa]
version: 1
visibility: public
metadata:
  clawdbot:
...
HoodGrow reads Robinhood Chain (chain id 4663) stock token contracts directly — the full catalog's live price, corporate-action adjusted supply (ERC-8056 `uiMultiplier()`, so numbers stay correct through stock splits, not just raw token balances), and both pending (on-chain staged) and historical (official Robinhood ledger) corporate actions. One call returns the full catalog. Pay-per-call in USDC over x402 — no account, no API key.
...
- **Max price:** $0.50 per call. If a 402 response quotes a higher amount, do NOT pay — stop and tell the user.
...
`GET https://www.hoodgrow.com/api/agent/tokens` — $0.50 per call

Returns every listed token's price (with source: Chainlink or Robinhood registry), 24h change, and corporate-action adjusted supply, plus:
...
No parameters — one call returns the full catalog. On first call (no prior payment), the endpoint returns `HTTP 402` with payment terms encoded in the `payment-required` response header; pay the quoted USDC amount on Base and retry with the payment proof to receive the JSON response.
